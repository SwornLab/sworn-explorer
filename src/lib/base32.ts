...
export const toBase32 = (input: Uint8Array) => {
	let output = '';
	let temp = 0;
	let bits = 0;
...
export const fromBase32 = (input: string) => {
	const output = [];
	let temp = 0;
	let bits = 0;
...
export const shake = (input: string) => Buffer.from(sha3.shake256(input, 512), 'hex');

export const addressFromShake = (input: Uint8Array) => {
...
export const calculateAddress = (input: string) => {
...
export const recoverAddress = (input: string) => {
	if (input.length !== 34) {
